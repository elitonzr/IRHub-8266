...
function handleWSMessage(event) {
  ...
  switch (data.type) {
    ...
    case "wifiPortal":
      alert(
        "Conecte-se à rede '" +
          (document.getElementById("name").textContent || "irhub8266") +
          "'",
      );
      break;
    case "wifiReset":
...
function updateOutputsWS(data) {
  ...
  if (ledDot) ledDot.className = "dot " + (data.led ? "green" : "red");
}
...
function rebootDevice() {
  if (confirm("Reiniciar?")) wsSend({ cmd: "reboot" });
}
function openWifiPortal() {
  if (confirm("Abrir portal WiFi?")) wsSend({ cmd: "wifiPortal" });
}
function resetWifi() {
  if (confirm("Resetar WiFi?")) wsSend({ cmd: "wifiReset" });
}
function resetConfig() {
  if (confirm("Reset total? Isso apagará todas as configurações."))
    wsSend({ cmd: "configReset" });
}
